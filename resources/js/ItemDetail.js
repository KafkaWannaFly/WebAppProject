...
function setupNavigatePath(item) {
    try {
        let tittle = document.querySelector("head title");
...
function setUpCarousel(item) {
    try {
        let carouselOrderList = document.getElementById("carouselOrderList");
        carouselOrderList.innerHTML = "";
        let carouselInner = document.getElementById("carouselInner");
        carouselInner.innerHTML = "";
        let carouselListItemModel = document.querySelector(".carouselListItemModel");
        let carouselImageDivModel = document.querySelector(".carouselImageDivModel");
        console.log(`Number of image: ${item.imagePath.length}`);
        for (let i = 0; i < item.imagePath.length; i++) {
            let li = document.importNode(carouselListItemModel, true);
...
function setUpDetailContent(item) {
    try {
        ...
        for (let i = 0; i < item.options.length; i++) {
            let option = document.importNode(document.querySelector(".itemOptionModel"), true);
